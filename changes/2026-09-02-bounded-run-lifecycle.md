...
- **What changed:** Model calls now have an absolute wall-clock deadline, while
  ...
  conflict. Failed segments preserve their failed attempt record but restore
  the persistent execution, interaction, or conversation to its real runnable
  state. AG-UI terminal events are emitted only after the claim is finalized.
...
- **Migration / breaking note:** No database migration. Defaults are a
  240-second model deadline, 270-second segment deadline, no completion-token
  cap, and a 5-second slow-cancellation diagnostic threshold. A provider
  `length` finish is now an `OUTPUT_LIMIT` failure instead of a successful
  partial result.
